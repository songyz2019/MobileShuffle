...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        self.inference_mode = inference_mode
        self.in_planes = min(90, int(90 * width_multipliers[0]))
        self.use_se = use_se
        self.num_conv_branches = num_conv_branches
        ...
        # 32 64 128 256
        # 48 96 192 384
        self.stage1 = self._make_stage(int(90 * width_multipliers[0]), num_blocks_per_stage[0],
                                       num_se_blocks=0)
        self.stage2 = self._make_stage(int(180 * width_multipliers[1]), num_blocks_per_stage[1],
                                       num_se_blocks=0)
        self.stage3 = self._make_stage(int(360 * width_multipliers[2]), num_blocks_per_stage[2],
                                       num_se_blocks=int(num_blocks_per_stage[2] // 2) if use_se else 0)
        self.stage4 = self._make_stage(int(720 * width_multipliers[3]), num_blocks_per_stage[3],
        ...
        # )
        self.conv5 = MobileOneBlock(in_channels=1440, out_channels=1024,
